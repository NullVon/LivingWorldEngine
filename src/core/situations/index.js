import { copy } from '../../infrastructure/records.js';
...
export function updateSituations(state, recordEvent) {
  let count = 0;
  for (const item of situations(state.world)) {
    ...
    if (s.important && !s.offered && !pending.some(op => op.type === 'opportunity' && op.entity === item.id)) {
      recordEvent({ type: 'situation.opportunity', data: {}, effects: [
        { type: 'learn', actor: s.opportunity.actor, claim: copy(s.opportunity.claim) },
        { type: 'opportunity', entity: item.id },
      ] });
      count++;
    }
